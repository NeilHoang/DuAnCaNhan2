                                                    ...
                                                    <div class="product_detail">
                                                        <div class="star_icon">
                                                            @if(\App\Http\Service\Image\ImageService::getStar($image->id)!=0)
                                                                @for($i=0;$i<\App\Http\Service\Image\ImageService::getStar($image->id);$i++)
                                                                    <i class="fa fa-star"></i>  @endfor
                                                                @for($i=0;$i<5-\App\Http\Service\Image\ImageService::getStar($image->id);$i++)
                                                                    <i class="fa fa-star-o"></i>  @endfor
                                                            @else No rate
                                                            @endif
                                                        </div>
                                                        <div class="product_button">
                                                            <div class="cart_details">
                                                                <a href="{{ route('cart.addToCart', $image->id) }}">Add
                                                                    to cart</a>
                                                            </div>
                                                            <div class="cart_details">
